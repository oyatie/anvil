...
1. Intake: GitHub webhook while `serve` is up, or CLI (`review`, `fix`, `certify`, `triage`, `enlist`, `heal-queue`, `reconcile`). The handler dispatches `pull_request`, `pull_request_review_comment`, `workflow_run`, and `merge_group`; `issue_comment` is forwarded by `serve` but never dispatched, so it is not an intake path today. CLI is the same loop: `certify` calls the review pipeline directly, gated enlist included. The bypass is elsewhere — the standalone `enlist` subcommand, `POST /api/enlist`, and the queue-healer re-enlist each call the enlister without checking `is_admissible()` ([#17](https://github.com/oyatie/anvil/issues/17)).
...
1. Close DocGuard. Published docs match `TOTAL_GATES` or the gate fails. [PR #12](https://github.com/oyatie/anvil/pull/12) (`fix/docguard-honest-corpus`) merged 2026-08-21 with [PR #13](https://github.com/oyatie/anvil/pull/13). Published counts are now derived, and residual drift fails the gate closed. [PR #11](https://github.com/oyatie/anvil/pull/11) is Copilot plan-only theater. Closed 2026-08-21, branch kept. Do not treat it as the close. README `.env` must list `oyatie/anvil` with the other two.
...
3. Architecture pages. ADR-0001 keeps its founding filename on purpose: the count at inception. Live count is `TOTAL_GATES`. Keep founding names historical, but never write the founding token on an owned page in either form — corpus sync rewrites the digit form and the word form alike, wherever they appear, including inside a filename it cannot tell apart from a claim. Name that page by its ADR id instead. Repair doctrine and this ADR if they drift.
...
- Enlist lock: Anvil enlists when admissible. Manager does not merge or approve. Live gap on both sides: the review pipeline is the only caller that checks `is_admissible()` — the standalone `enlist` subcommand, `POST /api/enlist`, and the queue healer do not ([#17](https://github.com/oyatie/anvil/issues/17)) — and with no telemetry endpoint configured `is_admissible()` cannot return true at all, so the gated path never fires while the three ungated ones can ([#19](https://github.com/oyatie/anvil/issues/19)).
