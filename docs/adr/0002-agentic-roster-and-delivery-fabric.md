...
5. Enlist: if `is_admissible()`, Anvil enlists. Manager does not merge or approve. Enlisting is not passive: `MergeEnlister` submits an APPROVE review on Anvil's behalf and then sets auto-merge on the PR. The approval body and the enlistment note are derived from the certification report, and neither is published for a pull request Anvil refuses to admit ([#18](https://github.com/oyatie/anvil/issues/18)).
