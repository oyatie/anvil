...
- Enlist lock: Anvil enlists when admissible. Manager does not merge or approve. Live gap on both sides: the review pipeline is the only caller that checks `is_admissible()` ([#17](https://github.com/oyatie/anvil/issues/17)), and with no telemetry endpoint configured `is_admissible()` cannot return true at all, so the one gated path never fires while the three ungated ones can ([#19](https://github.com/oyatie/anvil/issues/19)).
