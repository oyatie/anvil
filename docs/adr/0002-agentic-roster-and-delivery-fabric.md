...
- Live corpus on the trunk is `TOTAL_GATES`. The authority is the field list on `PreMergeCertificationReport`, never a number written on this page. The trunk is `dev`: work lands there and promotion runs `dev → staging → canary → production`, enforced by `.github/workflows/promotion-predecessor.yml`. Until that was adopted, work landed on `main` while `dev` — the default branch — sat 43 commits behind, so pull requests auto-targeting `dev` diffed against a stale base. `main` retains the history but no decided role in the ladder.
