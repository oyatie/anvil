...
ticket queue (cockpit, WS-07; GitHub issues interim). Milestone IDs are stable; a ws-file row
...
| H1-6 | **Machine identity**: Anvil runs as a GitHub App; loop-guard decides on principal, not string match (WS-11; #171) | daemon `gh api user` returns app slug ≠ `jason931225`; seeded Jason-comment fixture reaches the fixer | Security |
