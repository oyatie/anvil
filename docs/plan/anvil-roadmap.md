...
| WS-08 | [ws-08-gate-honesty.md](ws-08-gate-honesty.md) | #59 proxy gates; #19/RC-2 absent evidence; #53, #200 fabricated inputs | `Evaluated` type; fixture-or-no-registration; proxy-census ratchet; fabricated-literal meta-test |
| WS-09 | [ws-09-evidence-provenance.md](ws-09-evidence-provenance.md) | #149, #151 shared-clone/wrong-head; #200's fetch-boundary absence | `SubjectRoot`-typed reads; wrong-head refusal test; per-repo lock |
| WS-10 | [ws-10-untrusted-input.md](ws-10-untrusted-input.md) | #192, #199, RC-3 | `ContributorSupplied<T>`/`Untrusted` seam; injection corpus in CI |
...
| WS-12 | [ws-12-instrument-verification.md](ws-12-instrument-verification.md) | #179, RC-1, RC-6; #52's scan-only coverage; #199's fence-keyed scan (in-flight, #202) | scanner meta-guard; path-keyed-read ratchet (342 ↓, 2026-08-31 baseline); instrument seeded both directions |
...
that scan's own header at `tests/no_model_prompt_fences_contributor_text_by_hand_test.rs:1`
@ `1ce2121` (the file does not exist on dev) reads "ADVISORY, NOT A RATCHET", and its documented hole 2
is this very `queue_healer` case -- "zero backticks, so this scan called it green by absence (#199)".
On the fixer half, measured rather than asserted: reverting only #199's cited site leaves #202's
**fence** scan green -- it cannot see that site -- while #202's **label-coverage** assertion
(`tests/api_auth_and_prompt_delimiting_test.rs:1060`) goes red on `ReviewedPath` -- it and
`ProposedFix` sit inside the cited site (`engine.rs:33,38` @ `1ce2121`), occur nowhere else, and so
become declared-and-unused. `WorkingDiff` does **not**: it is at `engine.rs:183`, a different
function, and survives this seed. That is coverage detection, not fence detection. Seeded on
`refs/pull/202/head` @ `1ce2121` under
`cargo nextest run -E 'binary(api_auth_and_prompt_delimiting_test) + binary(no_model_prompt_fences_contributor_text_by_hand_test)'`
...
| Conformance drift | findings per managed repo per week, paired with inward results from shared analysis code | required proof: subject-bound inward and outward results plus failing controls; startup observations alone do not establish universal enforcement (doctrine §5, WS-02 H1-13) |
...
| 2026-09-01 | **Revision 5 is what shipped, and revision 4 was blocked — neither fact was in this log until now.** The row above describes rev 4 (250 lines) as the state; the merged file is rev 5 at **272**. Rev 4's own review returned three blockers, all self-introduced by the cut: (1) `git ls-files --error-unmatch`, added as a typo-catcher, exits 1 for any path **not already in the index** — which is every file a milestone creates, so `H1-8b` ("one file per gate under `fidelity/gates/`") could not start, and the only escape was naming a wider parent, i.e. the scope-widening the same section forbids; (2) the class rule "no form of any granted tool that writes" un-granted `git commit`, `rustfmt <path>` and `cargo build` ten lines after granting them, and foreclosed the charitable reading; (3) `grep -ic 'depends-on'` prints a `:0` line per file where `git grep -ic` prints nothing, so a published claim did not reproduce and trigger 8 would have halted an obedient agent at §4. **Why rev 4 shipped them:** the instrument was verified on the failure it was meant to catch and never on the case it should accept — invariant 5 applied in one direction is not invariant 5. Rev 5 fixes all three and restores four detectors the cut dropped while keeping their rules (the id-ownership grep, the bare-`H1` enumeration, the live-test exclusion proof, the `rev-parse HEAD` pair). Approved on the fifth review — an in-session review leaving no artifact on the PR, stated here rather than cited, which is the same gap row (b) above corrects — with **zero blocking findings**: every number reproduced at `65f71fd`, §1 hand-verified as a superset of all 24 command families the file orders, and two milestone walkthroughs landing correctly (`H1-13` reaches DONE; `H1-8b` stops at §6.4 before the first edit). Recorded here rather than absorbed, because a log that names only the revisions that worked is the defect its own first row exists to prevent | Post-merge correction |
| 2026-09-01 | **Two findings the roadmap owes a workstream, from the architectural review of the same file.** (a) `docs/plan/h1-execution-prompt.md` publishes normative claims while sitting **outside** `corpus_sync::OWNED` (`src/doc_guard/corpus_sync.rs:23` — README, doctrine, openapi, plus the ADR dirs), so by WS-14's own Ownership ratchet (which `WS14-H1b` is the milestone for) it is an unowned claim that goes stale in silence. (b) The decay is not hypothetical: `ws-13`'s `H1-11d` pins the remote-branch baseline at **167**; the same instrument (`git branch -r --format='%(refname:short)' \| grep -c '^origin/'`) returned **170** on 2026-09-01 and **172** on 2026-09-04 — three values in three days, which is the finding rather than a detail — a number this plan's own evidence file calls "deliberately not pinned … a number that cannot reproduce off one machine does not belong in a file whose first invariant is measure, never quote". §8 trigger 6 halts a run on it today. Both belong to WS-14; neither is fixed here | Architectural review |
...
| 2026-09-04 | **Living-document duty: findings #199 and #200 mapped, no new workstream.** Both were filed 2026-09-01 (multi-angle reviews of #196/#195) on the plan's merge day — `createdAt` 06:25Z, ~2.5h *before* #197 merged at 08:54Z (`gh issue view 199 --json createdAt`; `gh pr view 197 --json mergedAt`) but after its review freeze — so neither appears in §3 and the "no finding left unscheduled" claim had gone stale on arrival. #199 (the fixer's write-access turn takes the review comment raw; `queue_healer` same shape; the fence-keyed ratchet cannot see a fenceless site) → WS-10, as required red seeds for H1-5a's meta-test, + WS-12 for the RC-1 instrument half. #200 (`aggregate_fleet_overview` publishes healthy literals on failed queries; `github::fetch_merge_queue_depth` launders failure into `Ok(0)`) → WS-08's fabricated-input drain + WS-09's fetch-boundary-absence class. Neither warranted a new workstream: each is a new instance of an already-scheduled class landing inside a ratchet's blast radius (for #200's WS-09 half, the ratchet is written by this same PR (in `36de5da`) -- WS-08's and WS-10's pre-existed) — the census absorbing findings without minting an N+1 gate is the mechanism working, and a new workstream per instance would be the anti-pattern §3 forbids. Delivery-state drift recorded rather than §1 re-measured wholesale (§1 is dated 2026-08-31 by its own header): dev @ `48cf259` (`git rev-parse origin/dev`), open PRs **5** — #196, #202, #203, #205, #206 at 07:52Z (#204 closed unmerged and #206 opened while this row was being drafted — the enumeration is pinned to its timestamp precisely because it went stale once already during drafting; `gh pr list --state open --json number`), open issues **14** (`gh issue list --state open --json number -q length`) | Living-document amendment (this PR) |
| 2026-09-04 | **Model routing audited against the live CLIs; WS-08 given a usefulness threshold and a stopping rule; research.md +3 citations (90→93, window unchanged).** The audit was commissioned as a version bump (agy → Gemini 3.8 Flash) and found the routing layer is not wired: `git grep -n get_stage_fallback_chain -- src \| grep -v stage_router.rs` returns nothing, so all 31 rungs that name a model literally of `src/ai_driver/stage_router.rs` (`grep -c 'specific_model: Some' src/ai_driver/stage_router.rs`; the `GitOps` arm at `stage_router.rs:283` adds a 32nd config via `ModelExecutionConfig::default()`, which **does** name a model -- `provider.rs:74-86` sets `specific_model: Some(default_frontier_model())`, i.e. `"opus5"`, the very id this row flags as rejected -- so all 32 name a model and 31 name one literally. An earlier revision of this row said the 32nd names none, which was read off the arm rather than off `Default`) have **no production caller** — they are live in the integration suite, where `tests/issue_triage_routing_test.rs` calls the function four times and covers `AgenticStage::IssueTriage`, which the in-file `#[cfg(test)]` block does not; WS-16's replacement must therefore budget that test file rather than assume a clean deletion. Production routing is `main.rs:104`/`:107` (`Config::from_env()`, then `to_model_config()`) — env `AI_PROVIDER` (default `agy`) / `AI_MODEL` / `AGY_EFFORT` (default `high`) — resolving to `gemini-3.7-flash` @ high, which is valid and live. Measured against the installed CLIs (`resolved_model()` is passed verbatim to `--model`; no mapping layer): **`fable-5`, `opus-5` and `claude-3-7-sonnet` are all REJECTED** by the claude CLI (catalog serves `claude-opus-5`, `claude-sonnet-5`, and **both** `claude-fable-5` and `claude-fable-5-1`; note the three do not fail alike — `claude-3-7-sonnet` is rejected as *retired* ("retired on February 19, 2026", i.e. it is in the catalog) while the others are rejected as *absent*, a distinction any fallback classifier has to keep), as is `provider.rs:33`'s `"opus5"` Anthropic default — live whenever `AI_PROVIDER=claude`, and pinned by a test asserting that exact string. `gpt-6-astra` is unavailable on the ChatGPT Codex plan — and it appears in no source file, test or config in this tree (`git grep -n 'gpt-6'` matches only this log), so it is a candidate raised in session, not a model the code asks for. Three same-shape defects: (1) `router.rs:524` `run_ensemble_subscription` logs a four-model ensemble and makes **one** claude call, to the rejected id — a claim nothing checks; (2) `reasoning_effort` reaches a CLI only for agy (`agy_turn`), and for codex is set by the host's `~/.codex/config.toml` since `HOME` is on the exec inherit allowlist, so it is uncontrolled rather than inert — and agy *hard-errors* on `xhigh` (`valid: low, medium, high`); (3) `webhook/pipelines/review.rs:198` records spend as `"gemini-3.7-flash"` whatever ran, against a substring-keyed rate table (flash $1.5/M vs opus $30/M). The literal `exec::agent("agy"` appears 6× in `src/` (`git grep -o 'exec::agent("agy"' -- src \| wc -l`; the closing paren is not adjacent, so the fuller string greps 0); the 6th is `router.rs:489` inside the router itself, so **five** call sites bypass the router — `fixer/engine.rs`, `fixer/evaluator.rs`, `queue_healer.rs`, `ci_triager.rs`, `doc_guard/mod.rs` — so the fix loop, the thing that writes code onto a PR that then auto-merges, is single-provider with no fallback. **Registry re-measured the same day:** 73 entries — 42 `Heuristic`, 22 `Aspirational`, 7 `Partial`, **2 `Measured`**; 36 `blocked_on: None`, 37 blocked with 37 pairwise-distinct blocker *strings* — but distinctness of strings is not disjointness of dependencies, and a keyword pass over the same 37 finds `telemetry` in 4, `deploy` in 4, and `prometheus`/`opentelemetry`/`canary`/`signing` in 3 each, so the blocked set schedules by shared capability (an earlier draft of this row claimed disjointness from exact-string distinctness alone, against this plan's own measure-overlap-before-claiming-disjoint law); the shared cause is in the gaps — and the proxy count is **42 of 73 `Fidelity::Heuristic`**, read from the typed enum (`fidelity/mod.rs:55-58` defines `Heuristic` as "a proxy signal — a regex, a line count, a filename match"), not classified out of the free-text `gap` field. Three drafts of this row tried the latter and all three were wrong: the published pattern returned 34/11/5 against a claimed 26/10/3, a second pair shipped with no pattern at all, and the sets never partitioned. A number derived from prose by regex is a proxy; the typed field beside it was the thing. The drift ledger is **computed and dropped**, not dead: `gap_report` calls the non-`cfg(test)` `drift::against_the_proof_ledger()` (`fidelity/mod.rs:181`) — the "0 drifting by construction" defect was fixed in `7aceff9` on 2026-08-29 — but `publish/scorecard.rs:309` is the only production caller and reads `.unaudited` only, while `GapReport::summary()`, the one renderer of `drift.len()`, has no production caller. Publishing it is a **precondition** for anything consuming fidelity state. **This row's first draft asserted the ledger was dead, quoting the doc comment above `against_the_proof_ledger` — which narrates the pre-fix state in the present tense — instead of reading the function beneath it; the review caught it, and it is recorded because "measured, not quoted" governs state claims and not only counts.** WS-08 gains H1-15 (finding disposition on the M3 `Finding` type), H1-16 (usefulness ratio with a disable threshold — Tricorder's rule: a gate over threshold is disabled, not annotated; deletion was already representable via ARCHITECTURE.md §8's shrink-only delete list, the missing piece was the measured trigger) H1-18 (the horizon budget ratified by ticket, which is the operand the stopping rule below has no value for until it lands, and which cannot start before H1-15) and H1-17 (`KANI_STATUS` promoted by actually invoking Kani, domain scoped to `src/`, where the unsafe surface is empty today so the honest first result is a `Withheld` from a gate that ran — `blocked_on: None`, the corpus's highest-value single promotion). New ids start at H1-15 because H1-1..H1-14 are taken globally; `H1-7d`'s two-owner defect is already logged 2026-09-01 and is not re-raised here. **Stopping rule recorded as a decision, not a finding:** the drain is bounded by an error-budget-shaped target per horizon and stops when met, because "promote every gate" is unbounded and unbounded improvement is the N+1 loop WS-08 exists to prevent — and the six-month source window returned only undated vendor pages and posts dated 2026-01-30/2026-02-20 for it, so **no citation was added** and the provenance is named in `ws-08` instead. Routing table itself not yet built and **no file is committed for it here** — a config path cited before it exists is the dead-pointer class this log already records; the replacement for `stage_router.rs` lands with its loader, validator and dispatch classifier under WS-16, not ahead of them. Self-improvement is explicitly deferred by the user until the existing path is working, so no stage, milestone or capability for it is scheduled by this row | Model-lifecycle audit (WS-16) and gate-honesty amendment (WS-08) |
| 2026-09-06 | Correct doctrine §5 and its conformance references: startup logs Clean Architecture and shape observations, not universal inward or compile-time enforcement. Retain H1-13's inward obligation and exit criteria. Correct doctrine §6's dead-stage examples: proof and prevention ledgers have blocked-scorecard publication consumers, not admission gates | Promotion #217 source review against `b4d9844`; actual startup and publication call paths, not a new enforcement mechanism |
| 2026-09-06 | Reconcile WS-01's initial face-census/k8s filing record with the already recorded external-review correction; label the execution companion's revision-4 claim as rejected and carry the recorded revision-5 correction | Preserve historical evidence without claiming a fresh upstream measurement, issue resolution or missing review artifact; promotion #217 D1-3/D1-4 |
