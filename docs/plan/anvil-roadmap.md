...
| 2026-09-04 | **Model routing audited against the live CLIs; WS-08 given a usefulness threshold and a stopping rule; research.md +3 citations (90→93, window unchanged).** The audit was commissioned as a version bump (agy → Gemini 3.8 Flash) and found the routing layer is not wired: `git grep -n get_stage_fallback_chain -- src \| grep -v stage_router.rs` returns nothing, so all 31 rungs of `src/ai_driver/stage_router.rs` are dead code. Production routing is `main.rs:106` `Config::from_env().to_model_config()` — env `AI_PROVIDER` (default `agy`) / `AI_MODEL` / `AGY_EFFORT` (default `high`) — resolving to `gemini-3.7-flash` @ high, which is valid and live. Measured against the installed CLIs (`resolved_model()` is passed verbatim to `--model`; no mapping layer): **`fable-5`, `opus-5` and `claude-3-7-sonnet` are all REJECTED** by the claude CLI (real ids `claude-fable-5-1`, `claude-opus-5`, `claude-sonnet-5`), as is `provider.rs:33`'s `"opus5"` Anthropic default — live whenever `AI_PROVIDER=claude`, and pinned by a test asserting that exact string. `gpt-6-astra` is unavailable on the ChatGPT Codex plan. Three same-shape defects: (1) `router.rs:524` `run_ensemble_subscription` logs a four-model ensemble and makes **one** claude call, to the rejected id — a claim nothing checks; (2) `reasoning_effort` reaches a CLI only for agy (`agy_turn`), and for codex is set by the host's `~/.codex/config.toml` since `HOME` is on the exec inherit allowlist, so it is uncontrolled rather than inert — and agy *hard-errors* on `xhigh` (`valid: low, medium, high`); (3) `webhook/pipelines/review.rs:198` records spend as `"gemini-3.7-flash"` whatever ran, against a substring-keyed rate table (flash $1.5/M vs opus $30/M). Five call sites bypass the router entirely on a hardcoded `exec::agent("agy")` — `fixer/engine.rs`, `fixer/evaluator.rs`, `queue_healer.rs`, `ci_triager.rs`, `doc_guard/mod.rs` — so the fix loop, the thing that writes code onto a PR that then auto-merges, is single-provider with no fallback. **Registry re-measured the same day:** 73 entries — 42 `Heuristic`, 22 `Aspirational`, 7 `Partial`, **2 `Measured`**; 36 `blocked_on: None`, 37 blocked and **all 37 blockers distinct**, so there is no cluster to unlock; the shared cause is in the gaps (30 of 73 describe a string/regex standing in for the named instrument, 11 a hardcoded result). `fidelity/drift.rs` records that `gap_report` hardcodes `drift: Vec::new()` and every `audit_against_reality` caller is `#[cfg(test)]` — the drift ledger is dead in production, so it is a **precondition** for anything consuming fidelity state, not a milestone beside them. WS-08 gains H1-15 (finding disposition on the M3 `Finding` type), H1-16 (usefulness ratio with a disable threshold — Tricorder's rule: a gate over threshold is disabled, not annotated; deletion was already representable via ARCHITECTURE.md §8's shrink-only delete list, the missing piece was the measured trigger) and H1-17 (`KANI_STATUS` promoted by actually invoking Kani — `blocked_on: None`, the corpus's highest-value single promotion). New ids start at H1-15 because H1-1..H1-14 are taken globally; `H1-7d`'s two-owner defect is already logged 2026-09-01 and is not re-raised here. **Stopping rule recorded as a decision, not a finding:** the drain is bounded by an error-budget-shaped target per horizon and stops when met, because "promote every gate" is unbounded and unbounded improvement is the N+1 loop WS-08 exists to prevent — and the six-month source window returned only undated vendor pages and posts dated 2026-01-30/2026-02-20 for it, so **no citation was added** and the provenance is named in `ws-08` instead. Routing table itself not yet built and **no file is committed for it here** — a config path cited before it exists is the dead-pointer class this log already records; the replacement for `stage_router.rs` lands with its loader, validator and dispatch classifier under WS-16, not ahead of them. Self-improvement is explicitly deferred by the user until the existing path is working, so no stage, milestone or capability for it is scheduled by this row | Model-lifecycle audit (WS-16) and gate-honesty amendment (WS-08) |
