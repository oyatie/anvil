...
(`tests/api_auth_and_prompt_delimiting_test.rs:1060`) goes red, because `ReviewedPath`, `ProposedFix`
and `WorkingDiff` occur only in `src/fixer/engine.rs` and become declared-and-unused. That is coverage
detection, not fence detection. Seeded on `refs/pull/202/head` @ `1ce2121`: 46/46 pass unseeded,
45 pass / 1 fail on the narrowest seed. An earlier revision of this row claimed the fixer half
