...
pass, invariant I1, inside the test suite itself). Issue #179 measured 332 such reads across 34
files on 2026-08-30; re-measured at dev head `6128284` on 2026-08-31 the count is **342**
(`grep -rn 'src/[a-z_]*\.rs"' tests/*.rs | wc -l`).
...
| H1-7b | Path-keyed-read drain (#179): reads resolve subjects through module-aware discovery (split-safe), or `expect` loudly | baseline **342** (re-measured 2026-08-31 at dev head, same grep as #179 — it grew from 332 in one day, which is the front-loading argument in one number) ratcheted strictly downward; a new `read_to_string("src/…")` + `unwrap_or_default` is unwritable (meta-test, seeded) | Test infrastructure |
...
- Path-keyed-read count: monotone decreasing from 342 (2026-08-31 baseline); increase is merge-blocking.
...
No "better regexes" (the postmortem's explicit non-fix); no blanket rewrite of all 342 reads in one
PR (drain by module as subjects split, ratchet holds direction); scans stay for wiring assertions
