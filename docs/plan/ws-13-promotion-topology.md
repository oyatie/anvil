...
sprawl: 315 local (`git branch | wc -l`) / 166 origin (`git branch -r | grep -c '^origin/'`) / 78
`pr/*` refs (`git branch -r | grep -c '^pr/'`), 50 worktrees (`git worktree list | wc -l`).
...
- Branch-count ratchet: baseline frozen at the H1-11d census; growth past baseline is a red in the
  weekly sweep.
