...
pub struct AllowlistedDebt {
    ...
    pub debt_note: String,
}

/// The allowlist as it is stored on disk.
#[derive(Debug, Clone, serde::Deserialize)]
struct BrandAbsencePolicy {
    allowlisted_debt: Vec<AllowlistedDebt>,
}

/// Where the policy lives, relative to the crate root.
pub const POLICY_PATH: &str = "src/brand_absence/brand-absence-policy.json";

/// Reads the allowlist from disk.
///
/// The allowlist is DATA, not code. It was a compiled-in `&'static` array, which
/// meant the gate could not run against any repository but this one and could
/// not be updated without a rebuild -- contradicting oyatie's own stated pattern
/// for exactly this kind of gate ("the gate engine is UNIVERSAL: it reads this
/// allowlist as DATA and runs on ANY repo").
///
/// A missing or unreadable policy yields an EMPTY allowlist, not a permissive
/// one: every existing violation is then reported. A gate that cannot read its
/// exemptions must not behave as though everything is exempt.
pub fn load_allowlist(repo_root: &std::path::Path) -> Vec<AllowlistedDebt> {
    let path = repo_root.join(POLICY_PATH);
    let Ok(body) = std::fs::read_to_string(&path) else {
        return Vec::new();
    };
    serde_json::from_str::<BrandAbsencePolicy>(&body)
        .map(|p| p.allowlisted_debt)
        .unwrap_or_default()
}
...
impl BrandAbsenceReport {
    /// The verdict this gate publishes, decided here and carried unchanged.
    ///
    /// `src/trace_context_guard/mod.rs` is the precedent, and its reasoning is
    /// the reason this method exists: a gate that hands the evaluator a
    /// `Vec<BrandViolation>` and lets the wiring rebuild a severity from it has
    /// no say in its own severity. That is exactly what happened. The module
    /// declared itself advisory in [`BrandAbsenceReport::is_blocking`] and the
    /// evaluator published `GateStatus::Failed` on any new violation, so the
    /// severity a reader saw was stricter than the one the module computed, and
    /// the field that says otherwise was decoration.
    ///
    /// The severity tracks [`WARN_ONLY`] through `is_blocking`, so flipping that
    /// switch -- the documented way to make new violations fatal -- moves the
    /// published verdict with it and needs no second edit anywhere else.
    ///
    /// Advisory is the right level for what this gate is, not merely for what
    /// its ledger currently holds: [`BrandAbsenceGate::scan_tree`] is pointed at
    /// Anvil's own tree by `CARGO_MANIFEST_DIR`, so the verdict is a constant
    /// with respect to the pull request being certified. A finding whose subject
    ...
    pub fn gate_status(&self) -> crate::pre_merge_guard::report::GateStatus {
        ...
        if self.is_blocking {
            GateStatus::Failed(sentence)
        } else {
            GateStatus::Warning(sentence)
        }
    }
}
...
impl BrandAbsenceGate {
    ...
    pub fn new() -> Self {
        Self::with_allowlist(load_allowlist(std::path::Path::new(env!(
            "CARGO_MANIFEST_DIR"
        ))))
    }

    /// Gate backed by a caller-supplied ledger.
    pub fn with_allowlist(allowlist: Vec<AllowlistedDebt>) -> Self {
        Self {
            allowlist,
