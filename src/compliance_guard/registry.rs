...
impl TemporalValidity {
    pub fn is_currently_enforceable(&self, eval_date: &str) -> (bool, bool) {
        ...
        let is_sunset = self.sunset_date.as_ref().map(|s| eval_date > s.as_str()).unwrap_or(false);
        let in_grace_period = self.grace_period_until.as_ref().map(|g| eval_date <= g.as_str()).unwrap_or(false);
