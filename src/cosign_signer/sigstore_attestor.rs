...
impl SigstoreAttestor {
    ...
    pub fn sign_artifact_digest(&self, artifact_digest: &str) -> CosignSignatureBundle {
        CosignSignatureBundle {
            ...
            rekor_entry_uuid: format!("rekor-log-uuid-{}", &artifact_digest[..8.min(artifact_digest.len())]),
            is_valid: true,
        }
    }
}
...
mod tests {
    ...
    fn test_cosign_signing_bundle() {
        ...
        assert_eq!(bundle.oidc_issuer, "https://token.actions.githubusercontent.com");
        assert!(bundle.rekor_entry_uuid.starts_with("rekor-log-uuid-sha256:a"));
    }
}
