...
mod construction;
use construction::{command_in, trusted_provider_command};

/// Applies the posture through the private finite provider construction path.
fn command(tool: &str, posture: &Posture, framing: Framing) -> Result<AgentCommand> {
    command_in(tool, posture, framing, std::env::vars())
}
#[cfg(test)]
use construction::{prepare_command, trusted_provider_command_from};
