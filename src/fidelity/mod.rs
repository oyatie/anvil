...
pub use proof::{
    FAILURE_PROOFS, MEASURED_GATES_FLOOR, measured_without_proof, proofs_without_a_gate,
};
...
mod tests {
    ...
    fn the_report_admits_how_much_is_unaudited() {
        ...
        // that becomes real into a build failure.
        assert!(
            r.measured >= MEASURED_GATES_FLOOR,
            "{} gates are Measured; the floor is {} and may not fall",
            r.measured,
            MEASURED_GATES_FLOOR
        );
        assert!(
            r.honesty_ratio() > 0.0,
            "at least one gate measures what it claims"
        );
        // Every `Measured` gate names the proof that it can fail, and no proof
        // names a gate that is gone.
        assert!(
            measured_without_proof(registry::AUDITED_GATES, FAILURE_PROOFS).is_empty()
                && proofs_without_a_gate(registry::AUDITED_GATES, FAILURE_PROOFS).is_empty()
        );
    }
}
