...
pub async fn certify_pull_request(
    ...
    // Built here rather than taken as an argument: no caller can hand this
    // corpus the shared clone, because no caller hands it a tree at all.
    // See `GitManager::certified_tree_at`.
    let tree = state
        .git_mgr
...
async fn certify_for_enlistment(
    ...
    // For the clone's existence, not for its working tree. Nothing on this
    // path reads that tree any more; `certified_tree_at` produces the one the
    // gates measure.
    state
        .git_mgr
