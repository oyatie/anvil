...
/// Comment lines narrating a previous revision. EXACT, and it must fall.
const NARRATIVE_COMMENT_LINES: usize = 157;
...
#[test]
fn comments_narrating_a_previous_revision_are_exactly_what_was_recorded() {
    let found = offenders();
    assert_eq!(
        found.len(),
        NARRATIVE_COMMENT_LINES,
        "comment lines narrating history moved. Falling is the work: state the \
         contract in the doc comment and put the incident in the commit \
         message, where it is already recorded once and cannot go stale. \
         Rising means a second copy of a story was written into the source."
    );
}
